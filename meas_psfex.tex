...
The \texttt{meas\_extensions\_psfex} package provides an interface to the external PSFEx tool \citep{2011ASPC..442..435B} for modeling spatially varying point spread functions (PSFs).
...
The package raises specific exceptions for common failure modes: when no stars are available, when none pass quality cuts, or when too few good stars remain to support the required model complexity.
