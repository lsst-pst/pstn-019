...
In the LSST Science Pipelines, it specifically means assigning fractions of the flux from all pixels in a \texttt{Footprint} to each of the peaks within that \texttt{Footprint}, with each peak then considered a ``child'' source.
This fractional flux assignment is very different from creating a ``segmentation map'', which fully assigns each pixel to each child source, as done by, for example, Source Extractor \citep{1996A&AS..117..393B}.
