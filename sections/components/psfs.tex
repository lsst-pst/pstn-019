...
\texttt{PSFEx} is faster and is used in nightly alert processing  as well as to obtain a preliminary model in data release processing.
\texttt{Piff} is then used in a second, more detailed round of PSF estimation during data release processing to achieve better accuracy, especially for weak gravitational lensing.
...
At its core is a task which prepares selected stars for input into \texttt{PSFEx}, invokes the library, and converts the resulting model into an LSST-specific PSF object.
...
The implementation within  \texttt{meas\_extensions\_piff} does not exploit the full modularity of \texttt{Piff}; instead, it closely follows the method used for cosmic shear analysis, such as that in DES \citep{2021MNRAS.501.1282J,2025OJAp....8E..26S}, however it has been designed to make it easy to add support for more options as needed.
...
\citet{2021MNRAS.501.1282J} configure \texttt{Piff} to fit in sky coordinates with a world-coordinate system (WCS) transform that includes CCD distortions such as tree rings.
