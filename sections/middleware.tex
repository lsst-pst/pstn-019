...
To meet these requirements we developed a library called the Data Butler \citep[see e.g.,][]{2022SPIE12189E..11J,2025ASPC..538..142L}, generally referred to as simply the Butler, with the source code in a package named \texttt{daf\_butler}.

Internally, the Butler is implemented as a registry, a database keeping track of datasets, and a datastore, a storage system that can map a Butler dataset to a specific collection of bytes.
...
\texttt{band} & Waveband of interest (e.g., \emph{r})  \\
...
The data coordinate locates the dataset in a dimensional space, where dimensions are defined in terms that scientists understand.
Some commonly used dimensions are listed in Table~\ref{tab:dims}.
Each dataset is uniquely identified by specifying its dataset type, its run collection, and its coordinates, with Butler refusing to accept another dataset that matches all three of those values.
...
A deep coadd on a patch of sky would not have \texttt{exposure} dimensions and would instead be something like \texttt{instrument="LSSTCam", tract=105, patch=2, band="r", skymap="lsst\_cells\_v1"}, which would tell you exactly where it is located in the sky and in what waveband since you can calculate it from the tract, patch, band and skymap, where the skymap defines how tract and patch correspond to regions on the sky
...
\begin{figure*}
    ...
    % Use includegraphics to force more width for the figure.
    \includegraphics[width=\textwidth]{figures/middleware/pipe_viz_comcam_subset.pdf}
    \caption{
        Example pipeline visualization of two selected tasks from the \texttt{LSSTComCam/DRP-v2.yaml} pipeline.
        The diagram illustrates the flow of datasets between tasks, with dashed lines indicating prerequisite calibration inputs.
...
This class, which is required to implement a specific interface, is used by the system to isolate the instrument-specific from the pipeline-generic.
...
\begin{itemize}
\item LSSTCam \citep{10.71929/rubin/2571927,2024SPIE13096E..1SR,2010SPIE.7735E..0JK}, LSSTComCam \citep{10.71929/rubin/2561361}, LATISS \citep{10.71929/rubin/2571930,2020SPIE11452E..0UI}, and associated Rubin Observatory test stands, calibration instruments, and simulators.
\item Hyper-SuprimeCam (HSC) on the Subaru telescope \citep{2018PASJ...70S...1M}.
\item The Dark Energy Camera (DECam) on the CTIO Blanco telescope \citep{2015AJ....150..150F,2008SPIE.7014E..0ED}.
\item Canada-France-Hawaii Telescope's MegaPrime \citep{2003SPIE.4841...72B}.
\end{itemize}

All of these instruments use the default dimension universe.

Additionally, teams outside the project have developed \texttt{obs} packages to support Subaru's Prime Focus Spectrograph \citep{2024SPIE13096E..05T}, VISTA's VIRCAM \citep{2015A&A...575A..25S},
...
\subsection{Image Data Models and File Abstraction}

For DP2 we modified our file serialization system to make the files more approachable for external users and to improve performance when requesting subsets of a file from an object store.\citep{DMTN-339}
Additionally, as part of this work we changed the Python types such that a deep coadd and a visit image now have different specialized types and we are no longer trying to fit every image into a single type.
This separation of data model from serialization outputs gives us the flexibility to adopt different file formats in the future if the community begins to migrate from FITS to ASDF, for example, and will allow us to do format conversion on demand in the future (we currently support lossless conversion between FITS, HDF5, and JSON).
