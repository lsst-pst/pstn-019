...
Unlike the other packages described in this paper the Solar System Pipeline is not installed as part of the standard science pipelines software via the \texttt{lsst\_distrib} metapackage.
...
The high-level procedure is to link DIASource detections within a night (when on-sky motion is approximately linear) into \emph{tracklets}, to link these tracklets across multiple nights (into tracks) and to fit the tracks with an orbital model to identify those tracks that are consistent with an asteroid orbit.
...
\texttt{heliolinx} is written in C++, but provides a Python API including a standard \texttt{Task} API.

Candidate discoveries with a high degree of certainty, as well as re-observations of already known objects, are reported to the Minor Planet Center (MPC) using the observation submission pipeline.
...
To enable speedy retrieval (on order of 100\,msec or less) of all objects in a visit, we precompute on-sky locations of all solar system objects, fit Chebyshev polynomials, and build an efficient HEALpix-based index allowing for fast lookup.
